...
import org.telegram.ui.Reactions.SpacingHorizontalDecorator;

import java.util.ArrayList;

public class ReactionTabPagerView extends FrameLayout {
    ...
    public ReactionTabPagerView(@NonNull Context context) {
        super(context);
    ...
    private void fillTabs() {
        if (adapter != null && tabsView != null) {
            ...
            for (int i = 0; i < adapter.getItemCount(); i++) {
                tabsView.addTab(adapter.getItemId(i), adapter.getIcon(i), adapter.getItemTitle(i));
            }
        }
    }
    ...
    public abstract static class Adapter {
        ...
        public abstract Object getIcon(int position);
    }
    ...
    public static class TabsView extends FrameLayout {
        ...
        private static class Tab {
            ...
            public Tab(int i, Object icon, String value) {
                id = i;
                this.value = value;
                this.icon = icon;
            }

            public int getWidth(TextPaint textPaint) {
                int textWidth = (int) Math.ceil(textPaint.measureText(value));
                int paddingIconAndSpacingWidth = AndroidUtilities.dp(TabView.LEFT_PADDING + TabView.ICON_SIZE + TabView.SPACING + TabView.RIGHT_PADDING + TabView.INSET);
                tabWidth = textWidth + paddingIconAndSpacingWidth;
                return tabWidth;
            }
        ...
        public class TabView extends View {
            ...
            public void setTab(Tab tab, int position) {
                currentTab = tab;
                currentPosition = position;
                setContentDescription(tab.value);
                requestLayout();
            }
            ...
            protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
                final int contentWidth = currentTab.getWidth(textPaint);
                setMeasuredDimension(contentWidth, MeasureSpec.getSize(heightMeasureSpec));
            }
            ...
            protected void onDraw(Canvas canvas) {
                final float radius = getMeasuredHeight() / 2f;

                tabWidth = currentTab.tabWidth;

                float inset = AndroidUtilities.dp(INSET) / 2f;
                bound.left = 0;
                bound.right = bound.left + tabWidth ;
                bound.top = 0;
                bound.bottom = getMeasuredHeight();
                bound.inset(inset, inset);

                int backgroundColor = 0x1A378DD1;
                int selectedStrokeColor = 0xcc368dd0;
                int unselectedStrokeColor = Color.TRANSPARENT;
                int strokeColor;
                int otherStrokeColor;
                int id1;
                int id2;
                ...
                if (currentTab.id == id1) {
                    ...
                    otherStrokeColor = unselectedStrokeColor;
                } else {
                    strokeColor = unselectedStrokeColor;
                    otherStrokeColor = selectedStrokeColor;
                }

                tabPaint.setStyle(Paint.Style.FILL);
                tabPaint.setColor(backgroundColor);
                canvas.drawRoundRect(bound, radius, radius, tabPaint);

                if ((animatingIndicator || manualScrollingToId != -1) && (currentTab.id == id1 || currentTab.id == id2)) {
                    tabPaint.setColor(ColorUtils.blendARGB(otherStrokeColor, strokeColor, animatingIndicatorProgress));
                } else {
                    tabPaint.setColor(strokeColor);
                }

                tabPaint.setStyle(Paint.Style.STROKE);
                canvas.drawRoundRect(bound, radius, radius, tabPaint);

                if (currentTab.icon instanceof Drawable) {
                    Drawable iconDrawable = (Drawable) currentTab.icon;
                    imageReceiver.setImageBitmap(iconDrawable);
                } else if (currentTab.icon instanceof TLRPC.Document){
                    TLRPC.Document document = (TLRPC.Document) currentTab.icon;

                    TLRPC.PhotoSize thumb = FileLoader.getClosestPhotoSizeWithSize(document.thumbs, 90);
                    SvgHelper.SvgDrawable svgThumb = DocumentObject.getSvgThumb(document, Theme.key_windowBackgroundGray, 1.0f);
                    if (svgThumb != null) {
                        if (thumb != null) {
                            imageReceiver.setImage(ImageLocation.getForDocument(thumb, document), "32_32",  svgThumb,"webp", "react", 0);
                        } else {
                            imageReceiver.setImage(ImageLocation.getForDocument(document), "32_32", svgThumb, "webp","react", 0);
                        }
                    } else {
                        imageReceiver.setImage(ImageLocation.getForDocument(thumb, document), "32_32", null, "webp", "react", 0);
                    }
                }
                int horPadding = AndroidUtilities.dp(LEFT_PADDING);
                int spacing = AndroidUtilities.dp(SPACING);
                int iconSize = AndroidUtilities.dp(ICON_SIZE);
                int left = horPadding;
                int top = Math.round((bound.height() - iconSize) / 2f);
                int right = left + iconSize;
                int bottom = top + iconSize;

                imageReceiver.setImageCoords(left, top, iconSize, iconSize);
                imageReceiver.draw(canvas);
                right += spacing;
                int textX = right;
                textPaint.setColor(0xff378DD1);
                if (!TextUtils.equals(currentTab.value, currentText)) {
                    currentText = currentTab.value;
        ...
        public TabsView(Context context) {
            ...
            tabPaint.setStrokeWidth(AndroidUtilities.dp(1.5f));
            ...
            listView.setSelectorDrawableColor(0xcc368dd0);
            ...
            listView.setPadding(AndroidUtilities.dp(10), 4, AndroidUtilities.dp(9), AndroidUtilities.dp(10));
            listView.setSelectorRadius(AndroidUtilities.dp(22));
            ...
            listView.addItemDecoration(new SpacingHorizontalDecorator(AndroidUtilities.dp(6)));
            ...
            addView(listView, LayoutHelper.createFrame(LayoutHelper.MATCH_PARENT, 44));
        ...
        public void addTab(int id, Object icon, String value) {
            ...
            Tab tab = new Tab(id, icon, value);
            allTabsWidth += tab.getWidth(textPaint) + AndroidUtilities.dp(6);
            tabs.add(tab);
        }
        ...
        private void updateTabsWidths() {
            ...
            for (int a = 0, N = tabs.size(); a < N; a++) {
                int tabWidth = tabs.get(a).getWidth(textPaint);
                positionToWidth.put(a, tabWidth);
                positionToX.put(a, xOffset);
                xOffset += tabWidth;
            }
        }
